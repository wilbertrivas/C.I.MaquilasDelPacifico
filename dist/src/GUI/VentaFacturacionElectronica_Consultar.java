...
import Controladores.ControlDB_VentaFacturacionElectronica_AnulacionFactura;
import Controladores.ControlDB_VentaFacturacionElectronica_ParametrosGenerales;
...
public class VentaFacturacionElectronica_Consultar extends javax.swing.JPanel {
    ...
    String [] registro;  
    public VentaFacturacionElectronica_Consultar() throws SQLException {
        initComponents();  
    ...
    private void initComponents() {
        ...
        valorFacturacion = new javax.swing.JComboBox<>();
        ...
        valorFacturacion.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "DOLARES", "PESOS" }));
        add(valorFacturacion, new org.netbeans.lib.awtextra.AbsoluteConstraints(840, 170, 140, 30));
        ...
        jLabel8.setText("CONSULTAR VENTAS ELECTRONICAS");
        ...
        add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 200, 1500, 460));
    ...
    private void consultarVentasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_consultarVentasActionPerformed
        ...
        if(idVenta.isSelected()){
            if(editIdVenta.getText().equals("")){
                alertaGeneral.setText("Mensaje: El Código de Factura de Venta Electronica no puede estar Vacio");
                validarConsulta=false;
            }else{
                if(Script.equals("")){
                    Script += " [facturaVentaElectronica].[id] like '%"+editIdVenta.getText()+"%' ";
                }else{
                    Script += " and [facturaVentaElectronica].[id] like '%"+editIdVenta.getText()+"%' ";
                }
            }
        }
        if(nit.isSelected()){
            if(editNit.getText().equals("")){
                ...
                Script += " [facturaVentaElectronica].[cliente_nit] like '%"+editNit.getText()+"%'";
            }
        }
        if(nombre.isSelected()){
         ...
         if(Dex.isSelected()){
            if(editDex.getText().equals("")){
                ...
                if(Script.equals("")){
                    Script += " [facturaVentaElectronica].[dex] like '%"+editDex.getText()+"%' ";
                }else{
                    Script += " and [facturaVentaElectronica].[dex] like '%"+editDex.getText()+"%' ";
                }
            }
        }
         ...
         if(bodega.isSelected()){
            ...
            if(Script.equals("")){ 
                Script += " [facturaVentaElectronica].[bodegaFacturaVenta_id] = '"+idBodega+"'  ";
            }else{
                Script += "  and [facturaVentaElectronica].[bodegaFacturaVenta_id] = '"+idBodega+"'  "; 
            }
        } 
              ...
              try{
                ...
                String fechaVentaF=anoF+"-"+mesF+"-"+diaF;
                            if(Script.equals("")){
                                ...
                                Script += " [facturaVentaElectronica].[fechaFactura]  BETWEEN CAST('"+fechaVentaI+"' AS DATE) AND CAST('"+fechaVentaF+"' AS DATE) ";
                            }else{
                                 //Script += " and fechaCompra BETWEEN '"+fechaCompraI+"' and '"+fechaCompraF+"' ";
                                 Script += " AND [facturaVentaElectronica].[fechaFactura]  BETWEEN CAST('"+fechaVentaI+"' AS DATE) AND CAST('"+fechaVentaF+"' AS DATE) ";
                            }
              }catch(Exception e){
        ...
        if(validarConsulta){
            try {
                ...
                Logger.getLogger(VentaFacturacionElectronica_Consultar.class.getName()).log(Level.SEVERE, null, ex);
            ...
            if(fila1==-1){
                ...
                try{
                    ...
                    if(Bodega.equals("Extranjera")){
                        if(valorFacturacion.getSelectedItem().toString().equalsIgnoreCase("DOLARES")){
                            if(estado.equals("Activo")){
                                Generar_Reportes generarReportes = new Generar_Reportes();
                                generarReportes.generar_FacturaVentaElectronicaExtranjera(Integer.parseInt(id));
                            }else{
                                if(estado.equals("Anulada")){
                                    Generar_Reportes generarReportes = new Generar_Reportes();
                                    generarReportes.generar_FacturaVentaElectronicaExtranjeraAnulada(Integer.parseInt(id));
                                }
                            }
                        }else{
                            if(valorFacturacion.getSelectedItem().toString().equalsIgnoreCase("PESOS")){
                                if(estado.equals("Activo")){
                                    Generar_Reportes generarReportes = new Generar_Reportes();
                                    generarReportes.generar_FacturaVentaElectronicaExtranjera_Pesos(Integer.parseInt(id));
                                }else{
                                    if(estado.equals("Anulada")){
                                        Generar_Reportes generarReportes = new Generar_Reportes();
                                        generarReportes.generar_FacturaVentaElectronicaExtranjeraAnulada_Pesos(Integer.parseInt(id));
                                    }
                                }
                            }
                        }
                        ...
                        if(Bodega.equals("Nacional")){
                            if(estado.equals("Activo")){
                                Generar_Reportes generarReportes = new Generar_Reportes();
                                generarReportes.generar_FacturaVentaElectronicaNacional(Integer.parseInt(id));
                            }else{
                                if(estado.equals("Anulada")){
                                    Generar_Reportes generarReportes = new Generar_Reportes();
                                    generarReportes.generar_FacturaVentaElectronicaNacionalAnulada(Integer.parseInt(id));
                                }
                            }
                        }
                    }
    ...
    private javax.swing.JComboBox<String> valorFacturacion;
    // End of variables declaration//GEN-END:variables
    public void tabla(String script) throws SQLException, ParseException{
        registro = new String[11]; 
        modelo = new DefaultTableModel(null, titulo);
        ControlDB_VentaFacturacionElectronica_ParametrosGenerales com = new ControlDB_VentaFacturacionElectronica_ParametrosGenerales();
        ...
        if(alertaColor.isSelected()){
            for(int i =0; i< listadoVentas.size(); i++){
                if(diferenciaFecha(listadoVentas.get(i).getFechaFactura()).equals(editColorSelect.getSelectedItem().toString())){
                    ...
                    ControlDB_VentaFacturacionElectronica_AnulacionFactura controlDB_VentasAnulacion = new ControlDB_VentaFacturacionElectronica_AnulacionFactura();
