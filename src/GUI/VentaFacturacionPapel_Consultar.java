...
import Controladores.ControlDB_VentaFacturacionPapel_AnulacionFactura;
import Controladores.ControlDB_VentaFacturacionPapel_ParametrosGenerales;
...
public class VentaFacturacionPapel_Consultar extends javax.swing.JPanel {
    ...
    String [] registro;  
    public VentaFacturacionPapel_Consultar() throws SQLException {
        initComponents();  
    ...
    private void initComponents() {
        ...
        jLabel8.setText("CONSULTAR VENTAS A PAPEL");
    ...
    private void consultarVentasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_consultarVentasActionPerformed
        ...
        if(idVenta.isSelected()){
            if(editIdVenta.getText().equals("")){
                ...
                if(Script.equals("")){
                    Script += " [facturaVentaPapel].[id] like '%"+editIdVenta.getText()+"%' ";
                }else{
                    Script += " and [facturaVentaPapel].[id] like '%"+editIdVenta.getText()+"%' ";
                }
            }
        }
        if(nit.isSelected()){
            if(editNit.getText().equals("")){
                ...
                Script += " [facturaVentaPapel].[cliente_nit] like '%"+editNit.getText()+"%'";
            }
        }
        if(nombre.isSelected()){
         ...
         if(Dex.isSelected()){
            if(editDex.getText().equals("")){
                ...
                if(Script.equals("")){
                    Script += " [facturaVentaPapel].[dex] like '%"+editDex.getText()+"%' ";
                }else{
                    Script += " and [facturaVentaPapel].[dex] like '%"+editDex.getText()+"%' ";
                }
            }
        }
         ...
         if(bodega.isSelected()){
            ...
            if(Script.equals("")){ 
                Script += " [facturaVentaPapel].[bodegaFacturaVenta_id] = '"+idBodega+"'  ";
            }else{
                Script += "  and [facturaVentaPapel].[bodegaFacturaVenta_id] = '"+idBodega+"'  "; 
            }
        } 
              ...
              try{
                ...
                String fechaVentaF=anoF+"-"+mesF+"-"+diaF;
                            if(Script.equals("")){
                                ...
                                Script += " [facturaVentaPapel].[fechaFactura]  BETWEEN CAST('"+fechaVentaI+"' AS DATE) AND CAST('"+fechaVentaF+"' AS DATE) ";
                            }else{
                                 //Script += " and fechaCompra BETWEEN '"+fechaCompraI+"' and '"+fechaCompraF+"' ";
                                 Script += " AND [facturaVentaPapel].[fechaFactura]  BETWEEN CAST('"+fechaVentaI+"' AS DATE) AND CAST('"+fechaVentaF+"' AS DATE) ";
                            }
              }catch(Exception e){
        ...
        if(validarConsulta){
            try {
                ...
                Logger.getLogger(VentaFacturacionPapel_Consultar.class.getName()).log(Level.SEVERE, null, ex);
                    ...
                    if(Bodega.equals("Extranjera")){
                        if(valorFacturacion.getSelectedItem().toString().equalsIgnoreCase("DOLARES")){
                            if(estado.equals("Activo")){
                                ...
                                generarReportes.generar_FacturaVentaPapelExtranjera(Integer.parseInt(id));
                            }else{
                                if(estado.equals("Anulada")){
                                    Generar_Reportes generarReportes = new Generar_Reportes();
                                    generarReportes.generar_FacturaVentaPapelExtranjeraAnulada(Integer.parseInt(id));
                                }
                            }
                        }else{
                            if(valorFacturacion.getSelectedItem().toString().equalsIgnoreCase("PESOS")){
                                if(estado.equals("Activo")){
                                    Generar_Reportes generarReportes = new Generar_Reportes();
                                    generarReportes.generar_FacturaVentaPapelExtranjera_Pesos(Integer.parseInt(id));
                                }else{
                                    if(estado.equals("Anulada")){
                                        Generar_Reportes generarReportes = new Generar_Reportes();
                                        generarReportes.generar_FacturaVentaPapelExtranjeraAnulada_Pesos(Integer.parseInt(id));
                                    }
                                }
                            }
                        }
                        ...
                        if(Bodega.equals("Nacional")){
                            if(estado.equals("Activo")){
                                Generar_Reportes generarReportes = new Generar_Reportes();
                                generarReportes.generar_FacturaVentaPapelNacional(Integer.parseInt(id));
                            }else{
                                if(estado.equals("Anulada")){
                                    Generar_Reportes generarReportes = new Generar_Reportes();
                                    generarReportes.generar_FacturaVentaPapelNacionalAnulada(Integer.parseInt(id));
                                }
                            }
                        }
                    }
    ...
    public void tabla(String script) throws SQLException, ParseException{
        ...
        ControlDB_VentaFacturacionPapel_ParametrosGenerales com = new ControlDB_VentaFacturacionPapel_ParametrosGenerales();
        ...
        if(alertaColor.isSelected()){
            for(int i =0; i< listadoVentas.size(); i++){
                if(diferenciaFecha(listadoVentas.get(i).getFechaFactura()).equals(editColorSelect.getSelectedItem().toString())){
                    ...
                    ControlDB_VentaFacturacionPapel_AnulacionFactura controlDB_VentasAnulacion = new ControlDB_VentaFacturacionPapel_AnulacionFactura();
